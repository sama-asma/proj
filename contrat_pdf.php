...
class ContratPDF extends tFPDF {
    // Propriétés communes
    ...
    public function __construct($orientation = 'P', $unit = 'mm', $format = 'A4') {
        ...
        $this->AddFont('DejaVu', 'B', 'DejaVuSansCondensed-Bold.ttf', true);  // gras
    ...
    public function Header() {
        ...
        $this->Cell(160, 10, $this->companyName . ' - ' . $this->getContractTitle(), 0, 1, 'L', true);
        // Saut de ligne
        $this->Ln(15);
    }
    ...
    public function addPrimeDetails($primeBase, $reduction, $surcharge, $primeFinale, $coefficients = []) {
        ...
        $primeNet = $primeFinale / ((1 - ($reduction / 100)) * (1 + ($surcharge / 100)));
        // Réduction
        if ($reduction > 0) {
            $montantReduction = $primeNet * $reduction / 100;
            $primeNet -= $montantReduction;
        ...
        // Surcharge
        if ($surcharge > 0) {
            $montantSurcharge = $primeNet * $surcharge / 100;
